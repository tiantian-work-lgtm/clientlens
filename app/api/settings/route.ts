...
export async function PUT(request: Request) {
  try {
    ...
    const body = await request.json() as {
      providers?: Array<{ provider: IntegrationProvider; apiKey?: string; model?: string; enabled?: boolean }>;
      assignments?: { analysisProvider?: Provider; translationProvider?: Provider; knowledgeProvider?: Provider };
    };
    const allowed: IntegrationProvider[] = ["openai", "deepseek", "salesmartly"];
    ...
    const modelProviders: Provider[] = ["openai", "deepseek"];
    const assignments = {
      analysisProvider: modelProviders.includes(body.assignments?.analysisProvider as Provider) ? body.assignments?.analysisProvider : "openai",
      translationProvider: modelProviders.includes(body.assignments?.translationProvider as Provider) ? body.assignments?.translationProvider : "deepseek",
      knowledgeProvider: modelProviders.includes(body.assignments?.knowledgeProvider as Provider) ? body.assignments?.knowledgeProvider : "openai",
    };
