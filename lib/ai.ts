import type { AnalysisReport, BuyingDriver, CommunicationImprovement, ConfirmationItem, CustomerEmotionProfile, DealBlocker, DealDecisionMap, KnowledgeScriptReference, LegacyAnalysisModule, NextStepStrategy, Objection, ProductMention, Provider } from "./types";
...
export async function analyzeModuleWithProvider(provider: Provider, conversation: string, module: LegacyAnalysisModule, analysisContext?: unknown): Promise<AnalysisModuleResult | null> {
  ...
  const attachKnowledge = (value: AnalysisModuleResult) => {
    if (module !== "action") return value;
    const action = value as ActionModuleResult;
    return { ...action, knowledgeReferenceIds: [], knowledgeReferences: [] } as AnalysisModuleResult;
  };
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      for (const chunk of chunks) {
        ...
        const rawResult = await requestModuleOnce(config, provider, module, `${retryPrefix}${chunk}${upstreamContext}`, false, "");
...
export async function generateChecklistSuggestion(provider: Provider, conversation: string, item: string, mode: "hook" | "explain"): Promise<BilingualSuggestion | null> {
  ...
  const prompt = `${commonPrompt}\n${instruction}\n沿用客户使用的语言生成 text，并为其提供自然简体中文翻译 translation。不得读取或引用话术库。只输出包含 text 和 translation 的合法 JSON。\n\n对话：\n${conversation}`;
  const config = await getRuntimeProviderConfig(provider);
  if (!config) return null;
  if (provider === "openai") {
    const response = await fetch(`${config.baseUrl || "https://api.openai.com"}/v1/responses`, {
